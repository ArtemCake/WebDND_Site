...
creature_type_race_link = Table(
	...
	Index('ix_creature_type_race_type', 'creature_type_id'),Index('ix_creature_type_race_race', 'race_id'))
...
creature_type_link = Table(
	...
	Index('ix_creature_type_creature', 'creature_id'),	Index('ix_creature_type_type', 'type_id'))
...
class Creature(Base):
	...
	types: Mapped[list["CreatureType"]] = relationship("CreatureType", secondary="creature_type_link", back_populates="creatures", lazy="selectin")
	tags: Mapped[list["NPCTag"]] = relationship("NPCTag", secondary="creature_npctag_link", back_populates="creatures", lazy="selectin")
...
class NPC(Base):
	...
	creature_template_id: Mapped[PG_UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("bestiary.id", ondelete="SET NULL"), nullable=True, index=True)
	system_id: Mapped[PG_UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("game_systems.id", ondelete="CASCADE"), nullable=False, index=True)
	# Для Homebrew/Лора Мастера
	owner_id: Mapped[PG_UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
	lore_entry_id: Mapped[PG_UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("lore_entries.id", ondelete="SET NULL"), nullable=True, index=True)
	name: Mapped[str] = mapped_column(String(100), nullable=False)
...
class NPCTag(Base):
	...
	npcs: Mapped[list["NPC"]] = relationship("NPC", secondary="npc_npctag_link", back_populates="tags", lazy="selectin", cascade="all, delete-orphan", single_parent=True)
	creatures: Mapped[list["Creature"]] = relationship("Creature", secondary="creature_npctag_link", back_populates="tags", lazy="selectin", cascade="all, delete-orphan", single_parent=True)
...
class NpcTypeLink(Base):
	...
	npc_id: Mapped[PG_UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("npcs.id", ondelete="CASCADE"), primary_key=True)
	type_id: Mapped[PG_UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("npc_types.id", ondelete="CASCADE"), primary_key=True)
...
class LoreEntry(Base):
	...
	parent_entry_id: Mapped[PG_UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("lore_entries.id", ondelete="SET NULL"), nullable=True, index=True)
