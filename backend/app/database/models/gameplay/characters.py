...
class CharacterSheet(Base):
	...
	id: Mapped[PG_UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid4)
	player_id: Mapped[PG_UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
...
class CharacterInventory(Base):
	...
	sheet_id: Mapped[PG_UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("character_sheets.id", ondelete="CASCADE"), primary_key=True)
	item_id: Mapped[PG_UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("equipment.id", ondelete="CASCADE"), primary_key=True)
...
class CharacterSheetAbilityScore(Base):
	...
	id: Mapped[PG_UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid4)
	character_sheet_id: Mapped[PG_UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("character_sheets.id", ondelete="CASCADE"), nullable=False, index=True)
	characteristic_id: Mapped[PG_UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("characteristics.id", ondelete="RESTRICT"), nullable=False, index=True)
