...
class MapLayer(Base):
	...
	map_id: Mapped[PG_UUID] = mapped_column(
		PG_UUID(as_uuid=True), ForeignKey("campaign_maps.id", ondelete="CASCADE"), nullable=False, index=True
	)
...
class MapHex(Base):
	...
	layer_id: Mapped[PG_UUID] = mapped_column(
		PG_UUID(as_uuid=True), ForeignKey("map_layers.id", ondelete="CASCADE"), nullable=False, index=True
	)
...
class MapObject(Base):
	...
	object_template_id: Mapped[PG_UUID | None] = mapped_column(
...
class Token(Base):
	...
	current_map_id: Mapped[PG_UUID | None] = mapped_column(
		PG_UUID(as_uuid=True), ForeignKey("campaign_maps.id", ondelete="SET NULL"), nullable=True, index=True
	)

	# Ссылка на сущность (может быть игроком через sheet или монстром из бестиария)
	character_sheet_id: Mapped[PG_UUID | None] = mapped_column(
		PG_UUID(as_uuid=True), ForeignKey("character_sheets.id", ondelete="SET NULL"), nullable=True, index=True
	)
...
class SpatialAudioZone(Base):
	...
	creator_id: Mapped[PG_UUID | None] = mapped_column(
		PG_UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True
	)
