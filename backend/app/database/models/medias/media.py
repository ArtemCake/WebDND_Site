...
class UploadedAsset(Base):
	...
	job_id: Mapped[PG_UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("ai_generation_jobs.id", ondelete="SET NULL"), nullable=True, index=True)
	...
	job: Mapped["AIGenerationJob | None"] = relationship( "AIGenerationJob", back_populates="target_asset", foreign_keys=[job_id])
	equipment: Mapped["Equipment | None"] = relationship("Equipment", back_populates="assets")
