...
class UploadedAsset(Base):
	...
	generation_job_id: Mapped[PG_UUID | None] = mapped_column(
		PG_UUID(as_uuid=True), ForeignKey("ai_generation_jobs.id", ondelete="SET NULL"), nullable=True, index=True
	)
...
class AIGenerationJob(Base):
	...
	target_asset_id: Mapped[PG_UUID | None] = mapped_column(
		PG_UUID(as_uuid=True), ForeignKey("uploaded_assets.id", ondelete="SET NULL"), nullable=True, index=True
	)
